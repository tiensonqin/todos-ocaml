...
module Storage_codec = struct
  ...
  let int_of_transit_value = function
    ...
    | Transit.Int64 value ->
        if
          Int64.compare value (Int64.of_int Int.min_value) >= 0
  ...
  let transit_of_tuple_types types =
    Transit.Array (List.map types ~f:transit_of_value_type)
  ...
  let schema_attr_to_transit attr =
    ...
    Option.iter attr.unique ~f:(fun unique ->
        add "db/unique" (transit_of_unique unique));
  ...
  let tuple_attrs_of_transit = function
    | Transit.Array values | Transit.List values ->
        Some (List.filter_map values ~f:keyword_of_transit)
    | _ -> None
  ...
  let schema_attr_of_transit = function
    | Transit.Map props ->
        List.fold props ~init:schema_attr_default ~f:(fun schema (key, value) ->
            ...
            | Some "db/unique" ->
                { schema with unique = unique_of_transit value }
            | Some "db/index" ->
                {
                  schema with
                  indexed = Option.value (bool_of_transit value) ~default:false;
                }
            | Some "db/isComponent" ->
                {
                  schema with
                  is_component =
                    Option.value (bool_of_transit value) ~default:false;
                }
            | Some "db/noHistory" ->
                {
                  schema with
  ...
  let datoms_to_transit datoms =
    Transit.Array (List.map datoms ~f:datom_to_transit)

  let datoms_of_transit = function
    | Transit.Array datoms | Transit.List datoms ->
        List.map datoms ~f:datom_of_transit
    ...
    Transit.Map
      [
        ...
        ( Transit.Keyword "duplicate-datoms",
          datoms_to_transit root.storage_duplicate_datoms );
        (Transit.Keyword "max-addr", Transit.Int root.storage_max_addr);
        ( Transit.Keyword "branching-factor",
          Transit.Int root.storage_branching_factor );
        (Transit.Keyword "ref-type", transit_of_ref_type root.storage_ref_type);
      ]

  let storage_node_to_transit = function
    | PSet.Leaf datoms ->
        Transit.Map [ (Transit.Keyword "keys", datoms_to_transit datoms) ]
  ...
  let storage_root_of_transit entries =
    {
      ...
      storage_max_tx =
        int_of_transit "storage root :max-tx" (require_key "max-tx" entries);
      storage_eavt =
        address_of_transit "storage root :eavt" (require_key "eavt" entries);
      storage_aevt =
        address_of_transit "storage root :aevt" (require_key "aevt" entries);
      storage_avet =
        address_of_transit "storage root :avet" (require_key "avet" entries);
      ...
      storage_branching_factor =
        int_of_transit "storage root :branching-factor"
  ...
  let storage_tail_of_transit = function
    | Transit.Array groups | Transit.List groups ->
        List.map groups ~f:datoms_of_transit
  ...
  let payload_of_transit = function
    | Transit.Map entries ->
        if Option.is_some (lookup_transit_key "schema" entries) then
          Ds.Storage_root (storage_root_of_transit entries)
        else if Option.is_some (lookup_transit_key "keys" entries) then
          Storage_node (storage_node_of_transit entries)
