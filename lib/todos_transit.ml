...
let lookup_cache reader text =
  ...
  if index < 0 || index >= Array.length reader.cache
  then decode_error ("unknown cache code: " ^ text)
...
let rec read_string reader ?(remember_value = true) text =
  if is_cache_code text
  then read_string reader ~remember_value:false (lookup_cache reader text)
...
and read_tag reader text =
  ...
  if String.length text >= 3 && String.sub text 0 2 = "~#"
  then (
    if remember_value then remember reader text;
...
module Json = struct
  ...
  let expect parser expected =
    ...
    if not (Char.equal actual expected)
    then fail parser ("expected '" ^ String.make 1 expected ^ "'")
  ;;
...
let rec write = function
  ...
  | Map entries ->
    J_list
      (J_string "^ "
       :: Stdlib.List.concat
            (Stdlib.List.map (fun (key, value) -> [ write key; write value ]) entries))
  | Set values -> J_list [ J_string "~#set"; J_list (Stdlib.List.map write values) ]
  | List values -> J_list [ J_string "~#list"; J_list (Stdlib.List.map write values) ]
